...
public class HttpDriver {
	...
	public static String doGetForEncoder(String url,String para) throws Exception {
		httpClient = HttpClients.createDefault();
		String para1=URLEncoder.encode(para,"utf-8");
		HttpGet get = new HttpGet(ReadPro.getPropValue("base_url") + para1);
		respone = httpClient.execute(get);
		HttpEntity entity = respone.getEntity();
		String content = EntityUtils.toString(entity, "utf-8");
		EntityUtils.consume(entity);
	...
	public static String doGet(String url, Map<String, Object> user,CookieStore cookie) throws Exception {
		...
		CloseableHttpClient client = HttpClients.custom().setDefaultRequestConfig(gConfig).setDefaultCookieStore(cookie)
				.build();
//		String para = URLEncoder.encode(user.toString(), "UTF-8");
		String para = mapToString(user);
		HttpGet get = new HttpGet(ReadPro.getPropValue("base_url")+url + "?" + para);
