		...
		try {
			if(ReadProperties.getPropertyValue("enable_email").equals("true"))
			{
				...
				try {
					...
					{
						SendMail.sendEmail(toMail,emailTitle, emailContent);
						Log.info("email send to "+toMail+" success");
					}else
					{
						SendMail.sendEmail(ReadProperties.getPropertyValue("success_to_mail"),emailTitle, emailContent);
